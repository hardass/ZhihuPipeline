...
### 1. 碎片化阅读 vs. 深度系统学习
在地铁上、午休时或刷手机的**碎片时间**，我们常常会在知乎上读到非常惊艳的专业回答或深度技术专栏。这些文章蕴含着巨大的价值，往往需要我们在电脑前进行**二次消化、划线批注、关联引用与课题调研**。然而：
* 📱 **知乎移动端**只适合快速吞吐，极难进行严肃的笔记整理和知识关联。
* 🗑️ **“失效预警”**：优秀的回答随时面临被原作者删除、修改，或者因为政治/平台政策而遭遇 404，导致知识流失。

### 2. 完美的 “收件箱（Inbox）” 闭环工作流
本项目将你的**知乎线上收藏夹**改造成了个人知识库的 **“Inbox（收件箱）”**。它提供了一套优雅的闭环管理流：
1. **随手收藏 (Collect)**：在手机/网页端看到好内容，随手点一下收藏。
2. **一键同步 (Sync)**：回到书桌前运行 `./sync.sh`，文章自动本地化为排版精致的本地 Markdown 笔记（包含所有高清图及热门评论），归档至你的 **Obsidian 库**。
3. **自动清空 (Archive)**：同步成功的瞬间，程序会自动在知乎网页端把文章从活动收藏夹**移动到私密的 `archive` 归档收藏夹**。

通过这一机制，**你知乎的日常收藏夹始终保持“清空”的零负担状态**，而你的 **Obsidian 本地库**则沉淀为一个**完全离线、永久保存、可全文本搜索的个人专属知识网**。
